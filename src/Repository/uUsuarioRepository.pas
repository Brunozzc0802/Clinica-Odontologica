...
function TUsuarioRepository.VerificarUsuario(const Nome, Senha: string)
  : TUsuario;
begin
  Result := nil;
...
begin
  ...
  with dmUsuarios.queryUsu do begin
    Close;
    SQL.Text :=
      'SELECT id, nome, senha, ativo, grupo FROM usuarios WHERE ativo = FALSE';
    Open;
...
begin
  ...
  with dmUsuarios.queryUsu do begin
    Close;
    SQL.Text :=
      'SELECT id, nome, senha, ativo, grupo FROM usuarios WHERE ativo = TRUE';
    Open;
...
begin
  with dmUsuarios.queryUsu do begin
    Close;
    SQL.Text := 'INSERT INTO usuarios (nome, senha, ativo, grupo) ' +
