...
begin
  with dmUsuarios.queryProfissionais do begin
    Close;
    SQL.Text := 'UPDATE profissionais ' + 'SET nome = :nome, ' + 'cpf = :cpf, '
      + 'telefone = :telefone, ' + 'email = :email,' + 'cep = :cep, ' +
      'endereco = :endereco, ' + 'ativo = TRUE ' + 'WHERE id = :id';
...
begin
  ...
  with dmUsuarios.queryProfissionais do begin
    Close;
    SQL.Text :=
      'SELECT id, nome, cpf, telefone, email, cep, endereco  FROM profissionais WHERE ativo = FALSE';
    Open;
...
function TProfissionaisRepository.ListarProfissionais
  : TObjectList<TProfissionais>;
var
  Profissional: TProfissionais;
begin
  Result := TObjectList<TProfissionais>.Create(True);
  with dmUsuarios.queryProfissionais do begin
    Close;
    SQL.Text :=
      'SELECT id, nome, cpf, telefone, cep, email, endereco FROM profissionais WHERE ativo = TRUE';
    Open;
